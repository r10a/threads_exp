...
public:
    ...
    void waitUntil(Key key, std::chrono::time_point<Clock, Duration> deadline) noexcept;
...
template<class Clock, class Duration>
inline void EventCount::waitUntil(Key key, std::chrono::time_point<Clock, Duration> deadline) noexcept {
    while ((val_.load(std::memory_order_acquire) >> kEpochShift) == key.epoch_) {
        futexWaitUntil(reinterpret_cast<Futex<std::atomic> *>(&val_) + kEpochOffset, key.epoch_, deadline);
    }
    // memory_order_relaxed would suffice for correctness, but the faster
    // #waiters gets to 0, the less likely it is that we'll do spurious wakeups
    // (and thus system calls)
