#ifndef CRTQUEUE_CRTURNQUEUE_HPP
#define CRTQUEUE_CRTURNQUEUE_HPP
...
private:
    ...
    HazardPointers<Node> *hp;// TODO {shm, 3, maxThreads}; // We need three hazard pointers
    const int kHpTail = 0;
    const int kHpHead = 0;
    const int kHpNext = 1;
    const int kHpDeq = 2;

//  TODO  Node *sentinelNode = new Node(nullptr, 0);
    Node *sentinelNode;
    ...
    void initShm() {
//        shared_memory_object::remove(SHM_NM);
        //Remove shared memory on construction and destruction
        /*struct shm_remove {
            shm_remove() { shared_memory_object::remove(SHM_NM); }
...
public:
    CRTurnQueue(fixed_managed_shared_memory *shm, int maxThreads = MAX_THREADS) : maxThreads(maxThreads), shm(shm) {
        // new Node(nullptr, 0);
        sentinelNode = shm->construct<Node>(anonymous_instance)(nullptr, 0);
        hp = shm->construct<HazardPointers<Node>>(anonymous_instance)(shm, 3, maxThreads);
        ...
        for (int i = 0; i < maxThreads; i++) {
            ...
            deqself[i].store(shm->construct<Node>(anonymous_instance)(nullptr, 0), std::memory_order_relaxed);
            deqhelp[i].store(shm->construct<Node>(anonymous_instance)(nullptr, 0), std::memory_order_relaxed);
//          TODO  deqself[i].store(new Node(nullptr, 0), std::memory_order_relaxed);
//          TODO  deqhelp[i].store(new Node(nullptr, 0), std::memory_order_relaxed);
        }
    }


    ~CRTurnQueue() {
//      TODO  delete sentinelNode;
        shm->destroy_ptr(sentinelNode);
        while (dequeue(0) != nullptr); // Drain the queue
//      TODO  for (int i = 0; i < maxThreads; i++) delete deqself[i].load();
//      TODO for (int i = 0; i < maxThreads; i++) delete deqhelp[i].load();
        for (int i = 0; i < maxThreads; i++) shm->destroy_ptr(deqself[i].load());
        for (int i = 0; i < maxThreads; i++) shm->destroy_ptr(deqhelp[i].load());
    }
    ...
    void enqueue(T *item, const int tid) {
        ...
        Node *myNode = shm->construct<Node>(anonymous_instance)(item, tid);
//      TODO  Node *myNode = new Node(item, tid); TODO
...
#endif /* CRTQUEUE_CRTURNQUEUE_HPP */