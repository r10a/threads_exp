...
void *producer(void *ptr) {
    ...
    if ((qd_prod = mq_open(PROD, O_WRONLY | O_NONBLOCK | O_CREAT, QUEUE_PERMISSIONS, &attr)) == -1) {
...
void *consumer(void *ptr) {
    ...
    if ((qd_cons = mq_open(PROD, O_RDONLY | O_NONBLOCK | O_CREAT, QUEUE_PERMISSIONS, &attr)) == -1) {
